...
export default function Dashi() {
    ...
    const setuserupdatedata = async(id)=>{
        const response = await fetch(`https://newmail-2.onrender.com/propertybyid/${id}`);
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('https://newmail-2.onrender.com/getproperty/get');
    ...
    formDataToSend.append("propertyImg", formData.propertyImg);
        ...
        try {
            if (isEdit) {
                ...
                await axios.put(`https://newmail-2.onrender.com/property/update/${formData.id}`, formDataToSend);
            } else {

                await axios.post('https://newmail-2.onrender.com/property/add', formDataToSend);
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`https://newmail-2.onrender.com/property/delete/${id}`);
            fetchData();
        } catch (error) {
                        ...
                        <TableBody>
                            {data.map((row,index) => (
                                <TableRow key={row.id}>
                                    ...
                                    <TableCell>
                                     <img width={55} src={`https://newmail-2.onrender.com/uploads/${row.images}`} />
                                    </TableCell>
                ...
                <Dialog open={open} onClose={handleClose}>
                    <form onSubmit={handleSubmit}>
                        ...
                        <DialogContent>
                            ...
                            <TextField label="Price" fullWidth name="price" value={formData.price || ''} onChange={handleFormChange} style={{ marginBottom: '10px' }} />
